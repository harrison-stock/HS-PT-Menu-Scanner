(function () {
  ...
  // ── State ─────────────────────────────────────────────
  let saved = {};
  try { saved = JSON.parse(localStorage.getItem('ms_profile') || '{}'); } catch (e) {}

  const state = {
    screen: saved.name ? 'scanner' : 'landing',
    name: saved.name || '', email: saved.email || '',
    goal: saved.goal || null, cals: saved.cals || '',
    restrictions: saved.restrictions || [],
    touchedRestrictions: !!(saved.restrictions && saved.restrictions.length === 0 && saved.name),
  ...
  function persist() {
    const { name, email, goal, cals, restrictions, theme } = state;
    try { localStorage.setItem('ms_profile', JSON.stringify({ name, email, goal, cals, restrictions, theme })); } catch (e) {}
  }
  ...
  function render() {
    ...
    if (state.screen === 'profile') {
      ...
      $('settings-input-email').value = state.email;
  ...
  const actions = {
    ...
    'save-profile': () => {
      ...
      state.name = name; state.email = email; state.cals = $('input-cals').value.trim();
      if (!name) { state.err = 'Please enter your first name.'; render(); return; }
      if (!email.includes('@')) { state.err = 'Please enter a valid email address.'; render(); return; }
    ...
    'set-theme': (val, btn) => {
      state.theme = val; applyTheme(); persist(); render();
    },
    'save-settings': () => {
      state.name = $('settings-input-name').value.trim();
      state.email = $('settings-input-email').value.trim();
  ...
  $('input-name').addEventListener('input', (e) => { state.name = e.target.value; state.err = null; });
  $('input-email').addEventListener('input', (e) => { state.email = e.target.value; state.err = null; });
  ...
  $('settings-input-email').addEventListener('input', (e) => { state.email = e.target.value; });
